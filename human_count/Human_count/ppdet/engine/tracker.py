...
class Tracker(object):
    def __init__(self, cfg, mode='eval'):
        ...
        # build model
        self.model = create(cfg.architecture)  # fairmot   # ckpt model
    ...
    def load_weights_sde(self, det_weights, reid_weights):  # ckpt load weights
    ...
    def _eval_seq_jde(self,
        ...
        tracker = self.model.tracker
        tracker.max_time_lost = int(frame_rate / 30.0 * tracker.track_buffer)  # super params
        ...
        for step_id, data in enumerate(tqdm(dataloader)): # ckpt forawrd process
            self.status['step_id'] = step_id
            # forward
            timer.tic()
    ...
    def mot_predict_seq(self,
        ...
        dataloader = create('TestMOTReader')(self.dataset, 0)   # ckpt paddle sets
        result_filename = os.path.join(result_root, '{}.txt'.format(seq))
        if frame_rate == -1:
