...
parser.add_argument("--image_file", type=str,default= "/home/cata/Desktop/PaddleDetection/demo/ped3.png")
...
def predict_image(infer_config, predictor, img_list):
    ...
    for img_path in img_list:
        ...
        print("see transforms",transforms)
        inputs_name = [var.name for var in predictor.get_inputs()]
        inputs = {k: inputs[k][None, ] for k in inputs_name}
        print("in_feed:",inputs)
        outputs = predictor.run(output_names=None, input_feed=inputs)
        tem_im = Image.open(img_path).convert('RGB')
        ...
        if infer_config.arch in ["HRNet"]:
            ...
            for bbox in bboxes:
                if bbox[0] > -1 and bbox[1] > infer_config.draw_threshold:
                    draw.rectangle(list(bbox[2:]), outline='red', )

                    print(f"{int(bbox[0])} {bbox[1]} "
                          f"{bbox[2]} {bbox[3]} {bbox[4]} {bbox[5]}")
        tem_im.save('texture9.jpg')
