...
@register
@serializable
class JDETracker(object):   # ckpt  jde model
    ...
    def update(self, pred_dets, pred_embs=None):
        ...
        for cls_id in range(self.num_classes):  # select embeddings
        ...
        for cls_id in range(self.num_classes):
            ...
            pred_dets_cls = pred_dets_dict[cls_id] # [500,6]
            pred_embs_cls = pred_embs_dict[cls_id] # [500,1]
            remain_inds = (pred_dets_cls[:, 1:2] > self.conf_thres).squeeze(-1)  # [500,]
            if remain_inds.sum() > 0:
                pred_dets_cls = pred_dets_cls[remain_inds]
                if pred_embs_cls is None:  #  no-use reid
                    # in original ByteTrack
                    detections = [
                        STrack(
                            STrack.tlbr_to_tlwh(tlbrs[2:6]),  # [x1,y1,x2,t2] 2 [x1,y1,w,h]
                            tlbrs[1],
                            cls_id,
                            30,
            ...
            for track in self.tracked_tracks_dict[cls_id]:
                if not track.is_activated:
                    # previous tracks which are not active in the current frame are added in unconfirmed list  # uk
                    unconfirmed_dict[cls_id].append(track)   # not activated yet
                else:
            ...
            track_pool_dict[cls_id] = joint_stracks(
                tracked_tracks_dict[cls_id], self.lost_tracks_dict[cls_id])  # todo
            ...
            if pred_embs_cls is None:
                # in original ByteTrack
                dists = matching.iou_distance(track_pool_dict[cls_id],   # iou distance
                                              detections)
                matches, u_track, u_detection = matching.linear_assignment(   # XYL-M
                ...
                dists = matching.fuse_motion(             #  embedding + motion dist
